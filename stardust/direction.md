...
- Impeccable ignore set (2026-09-21, run close): value ignore added — design-system-font `roboto-fallback` (deploy Step 4 metric-matched fallback). NOT ignored: `layout-transition` on blocks/filters/filters.css L115/L216 and blocks/accordion/accordion.css L118 — the hook offers only a file-scoped `"*" --file` ignore, which the replica policy forbids on blocks/**; the three `transition: max-height …` rules are lifted verbatim from the source's accordion CSS (motion-observe evidence) and stand as disclosed, sanctioned exceptions · resolved by: hands-off
