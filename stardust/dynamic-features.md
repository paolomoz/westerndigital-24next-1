...
| 14 | m-modal-trigger-wd-modal-btn-chrome-only-target-outside-dom- | chrome modal trigger wd-modal-btn (sign-in / promo modal) | M | 1/4 | rebuild-native | self | done (header block: drawer + drill, nav drops, sign-in flyout, search expand — chrome cells gated in replica; parity.json #14) | chrome-interaction | none | motion/<slug>.json |
| 15 | m-tabs-expanders-role-tablist-aria-expanded-controls | home "Shop by Category / Solutions / Industries" tablist + 3 expanders | M | 1/4 | rebuild-native | self | done (tabs block; probe: click flips aria-selected within 800 ms; parity.json #15) | modal-loader | none | index sidecar |
...
| 26 | v-video-youtube | YouTube video | V | reach 4/4 | embed-passthrough | self | interim — no captured page renders a player; nothing to embed (parity.json #26) | media-as-url | none | all sidecars |
